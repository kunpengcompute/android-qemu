...
public:
    ...
    friend std::ostream& operator<<(std::ostream& out, const FbConfig& config);
private:
    FbConfig();
...
public:
    ...
    size_t getAllConfigCount() const { return static_cast<size_t>(mAllConfigCount); }

    const FbConfig* get(EGLConfig config, int& index) const;
    ...
    void getPackInfo(EGLint* numConfigs, EGLint* numAttribs) const;
    ...
    EGLint getMatchConfigs(EGLint hostConfig) const;
private:
    FbConfigList();
